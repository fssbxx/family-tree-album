...
#### 新增功能
- **头像从家庭相册选择**：设置头像时可以从家庭相册中选择照片
  - 头像选择对话框新增"个人相册"和"家庭相册"标签切换
  - 支持从任意相册选择照片作为头像
  - 裁剪后的头像仍保存到个人相册
- **移动端触摸交互支持**：族谱画布支持移动端手势操作
  - 单指拖动：平移族谱视图
  - 双指捏合：缩放族谱（最小30%，最大300%）

#### Bug 修复
- **修复照片显示失败**：修复 el-image 和 img 标签直接使用路径导致照片无法加载的问题
  - 添加 `getPhotoUrl()` 辅助函数统一处理图片 URL
  - 开发环境自动拼接完整 URL，生产环境使用相对路径
- **修复头像设置后不显示**：修复头像 URL 重复拼接 `/photos/` 前缀的问题
- **修复照片路径问题**：`getMemberPhotos` 使用 `sanitizeName` 处理成员名称，与上传时保持一致

#### 优化改进
- **移动端抽屉高度**：从 40vh 增加到 50vh，显示更多内容
- **移动端性别选择布局**：缩小头像尺寸，确保性别选择在一行显示
- **移动端添加按钮布局**：减小按钮间距和字体，确保三个按钮在一行完整显示
- **头像裁剪框高度**：减小约 100px，确保按钮完整显示在屏幕上
- **头像选择照片网格**：修复照片重叠问题，使用 padding-bottom 技巧维持宽高比

### v1.0.3 (2026-02-18)

#### 安全增强
- **登录速率限制**：同一 IP 连续 5 次密码错误后锁定 15 分钟，防止暴力破解

#### Bug 修复
- **修复批量上传照片失败**：临时文件名添加随机字符串，避免同一毫秒上传多文件时文件名冲突
- **修复家庭相册批量上传**：临时文件在所有处理完成后统一删除，避免重复复制失败
- **修复删除家庭照片失败**：修正前端请求参数传递方式
- **修复删除成员时照片未删除**：删除成员时同步删除其照片文件夹
- **修复头像修改后不刷新**：添加时间戳强制刷新缓存，同步更新本地数据
- **修复登录成功提示重复**：添加防抖检查

#### 优化改进
- **相册布局优化**：改为每行 3 张照片的网格布局
- **个人卡片高度调整**：高度调整为 166px，同步修复连接线和折叠按钮位置
- **登录页面简化**：移除密码权限说明提示
